from .uniformer_model import UNIFORMER_MODEL
import os
import copy
import numpy as np
...
class UniFormerV2:
    ...
    def __init__(self):
        ...
        self.model_mappings = {
            "UniFormer-V2-B": {"config": "models/uniformer/helpers/uniformerv2-base-p16-res224_clip-kinetics710-pre_8xb32-u8_kinetics400-rgb.py",
                               "checkpoint": "https://download.openmmlab.com/mmaction/v1.0/recognition/uniformerv2/uniformerv2-base-p16-res224_clip-kinetics710-pre_8xb32-u8_kinetics400-rgb/uniformerv2-base-p16-res224_clip-kinetics710-pre_8xb32-u8_kinetics400-rgb_20230313-75be0806.pth"
                               },
            "UniFormer-V2-L": {"config": "models/uniformer/helpers/uniformerv2-large-p14-res224_clip-kinetics710-pre_u32_kinetics400-rgb.py",
                               "checkpoint": "https://download.openmmlab.com/mmaction/v1.0/recognition/uniformerv2/kinetics400/uniformerv2-large-p14-res224_clip-kinetics710-pre_u32_kinetics400-rgb_20221219-56a46f64.pth"
                               }
        }
    ...
    def preprocess_fn(self, input_data, fps=None):
        """
        Preprocesses input data for Uniformer.
        ...
        required_num_frames = 32 if "-L" in self.identifier else 8
        if num_frames < required_num_frames:
            # fallback blank frame
            last_frame = frames[-1] if frames else Image.new("RGB", (256, 256))
            frames += [last_frame] * (required_num_frames - num_frames)
        elif num_frames > required_num_frames:
            indices = np.linspace(
                0, num_frames - 1, required_num_frames, dtype=int)
            frames = [frames[i] for i in indices]
        ...
        data = {'imgs': frames, 'num_clips': 1,
                'modality': 'RGB', 'clip_len': required_num_frames}
        data = processor(data)
        return data
    ...
    def postprocess_fn(self, features_np):
        ...
        """
        batch_size = int(features_np.shape[2] / 32)  # (257, 1, 32, 1024)
